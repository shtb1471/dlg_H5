...
(function ($) {
    $.hunterTimePicker = function (box, options) {
        ...
        function buildMinuteTpl(cur_time) {
            ...
            cur_time.find('.Hunter-minute-wrap').html(minute_html).css('right','10px').show();
        };
        ...
        function hourEvent() {
            time_wrap.on('click', '.Hunter-hour', function (event) {
                ...
                var temp = box.html().split(":");
                if (temp.length > 1) {
                    box.html(hourValue + ":" + temp[1]);
                } else {
                    box.html(hourValue + ":00");
                }
        ...
        function minuteEvent() {
            time_wrap.on('click', '.Hunter-minute', function (event) {
                ...
                var temp = box.html().split(":")[0] + ":" + minuteValue;
                box.html(temp);
                template.remove();
